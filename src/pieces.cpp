...
std::vector<std::array<int, 2>> Piece::rookMoves() const
{
	...
	int yPos { m_currentPos[0] };

	for (int xCurrent {xPos}; xCurrent < 8; xCurrent++)
	{
		allLegalMoves.push_back({xCurrent, yPos});
	}

	for (int xCurrent {xPos}; xCurrent >= 0; xCurrent--)
	{
		allLegalMoves.push_back({xCurrent, yPos});
	}

	for (int yCurrent {yPos}; yCurrent < 8; yCurrent++)
	{
		allLegalMoves.push_back({xPos, yCurrent});
	}

	for (int yCurrent {yPos}; yCurrent < 0; yCurrent--)
	{
		allLegalMoves.push_back({xPos, yCurrent});
	}    

	std::set<std::array<int, 2>> uniqueMoves(allLegalMoves.begin(), allLegalMoves.end());
  allLegalMoves.assign(uniqueMoves.begin(), uniqueMoves.end());
...
std::vector<std::array<int, 2>> Piece::bishopMoves() const
{
	...
	for (int xCurrent { xPos }, yCurrent { yPos }; xCurrent < 8 && yCurrent < 8; xCurrent++, yCurrent++)
	{
		allLegalMoves.push_back({xCurrent, yCurrent});
	}

	for (int xCurrent { xPos }, yCurrent { yPos }; xCurrent < 8 && yCurrent >= 0; xCurrent++, yCurrent--)
	{
		allLegalMoves.push_back({xCurrent, yCurrent});
	}

	for (int xCurrent { xPos }, yCurrent { yPos }; xCurrent >= 0 && yCurrent >= 0; xCurrent--, yCurrent--)
	{
		allLegalMoves.push_back({xCurrent, yCurrent});
	}

	for (int xCurrent { xPos }, yCurrent { yPos }; xCurrent >= 0 && yCurrent < 8; xCurrent--, yCurrent++)
	{
		allLegalMoves.push_back({xCurrent, yCurrent});
	}
